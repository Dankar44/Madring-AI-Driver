...
// ⤢ expand: the map widens over the panels' half of the dashboard. The page header stays put, so its
// controls remain within reach and there is no second copy of them anywhere.
const expandBtn=document.querySelector('#expandMap');
let mapExpanded=false;
function setMapExpanded(on){
  mapExpanded=on;document.body.classList.toggle('map-expanded',on);
  if(expandBtn){expandBtn.textContent=on?'⤡':'⤢';expandBtn.title=expandBtn.ariaLabel=on?'Reducir el mapa':'Ensanchar el mapa';}
  resizeSim();
}
if(expandBtn){
  expandBtn.addEventListener('click',()=>setMapExpanded(!mapExpanded));
  document.addEventListener('keydown',e=>{if(e.key==='Escape'&&mapExpanded)setMapExpanded(false);});
}
