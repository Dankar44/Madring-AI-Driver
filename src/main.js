...
const TRACK_SCALE = 1.32;
const MEMORY_KEY = 'madring-ai-driver-memories-v1';
const AUTO_MEMORY_GENERATIONS = new Set([1, 5, 10, 25, 50, 100]);

// Vector trace of the supplied MADRING silhouette. We deliberately scale the
// centreline up while keeping the road width almost unchanged: nearby parts of
// the circuit remain visually and physically separated instead of merging.
...
const track = {
  center: BASE_TRACK_CENTER.map(([x,y]) => [
    120 + (x - 110) * TRACK_SCALE,
    120 + (y - 170) * TRACK_SCALE,
  ]),
  halfWidth: 25,
};
...
class Car {
  ...
  sense(){
    return this.sensorDistances().map(d=>d/SENSOR_RANGE);
  }
  update(){
  ...
  drawFormulaCar(alpha=1){
    const body=this.champion?'#f4f2ed':'#80c9da';
    ...
    ctx.fillRect(-5.8,-7.0,4.6,3.2);
    ctx.fillRect(-5.8,3.8,4.6,3.2);
    ctx.fillRect(4.0,-6.5,4.2,3.0);
    ctx.fillRect(4.0,3.5,4.2,3.0);

    ctx.fillStyle='#24282b';
    ctx.fillRect(-9.4,-7.1,2.1,14.2);
    ctx.fillRect(8.0,-7.0,1.7,14.0);
    ctx.fillStyle=accent;
    ctx.fillRect(8.4,-6.7,.7,13.4);
    ...
    ctx.beginPath();
    ctx.ellipse(.4,0,2.4,1.8,0,0,Math.PI*2);
    ctx.fill();
    ctx.strokeStyle=accent;
    ctx.lineWidth=.75;
    ctx.beginPath();
    ctx.arc(.55,0,2.0,-1.1,1.1);
    ctx.stroke();
  ...
  draw(alpha=.25,showSensors=false){
    ...
    if(showSensors){
      ...
      for(let i=0;i<SENSOR_ANGLES.length;i++){
        ...
        ctx.lineWidth=1.6;
        ctx.strokeStyle='rgba(34,30,28,.85)';
        ctx.fillStyle='#fff';
